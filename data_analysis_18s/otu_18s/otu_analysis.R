...
## Then, the data of phyloseq_object_final is first transformed into relative abundance data and stored under phy_transformed.
...
pielou_2 <- ceiling(ifelse(test = alpha_lm_algae_genus$pielou > 0.8774688, yes = alpha_lm_algae_genus$pielou + 0.01, no = alpha_lm_algae_genus$pielou))
...
pielou_4 <- ceiling(ifelse(test = alpha_lm_protist_genus$pielou > 0.8974600, yes = alpha_lm_protist_genus$pielou + 0.01, no = alpha_lm_protist_genus$pielou))
...
### For Phragmoplastophyta and Ochrophyta
manova_abundant_algae_phylum <- manova(cbind(Phragmoplastophyta, Ochrophyta) ~ nutrient*sediment*flow*time, data = manova_df_algae_phylum_1)
...
### For Cymbella, Cocconeis, Messastrum, Navicula, Monoraphidium, Chlorococcum, Chlamydomonas, Gomphonema, Pseudellipsoidion, Spumella and Desmodesmus
manova_abundant_algae_genus <- manova(cbind(Cymbella, Cocconeis, Messastrum, Navicula, Monoraphidium, Chlorococcum, Chlamydomonas, Gomphonema, Pseudellipsoidion, Spumella, Desmodesmus) ~ nutrient*sediment*flow*time, data = manova_df_algae_genus_1)
...
### For Apicomplexa, Cercozoa, Tubulinea and Protosteliida
manova_abundant_protist_phylum <- manova(cbind(Apicomplexa, Cercozoa, Tubulinea, Protosteliida) ~ nutrient*sediment*flow*time, data = manova_df_protist_phylum_1)
...
### For Vermamoeba, Paracercomonas, Rhogostoma, Ichthyophthirius, Anteholosticha, Rhogostoma.1, Oikomonas, Uroleptus, Litonotus and Oxytricha  
manova_abundant_protist_genus <- manova(cbind(Vermamoeba, Paracercomonas, Rhogostoma, Ichthyophthirius, Anteholosticha, Rhogostoma.1, Oikomonas, Uroleptus, Litonotus, Oxytricha) ~ nutrient*sediment*flow*time, data = manova_df_protist_genus_1)
...
mydata_fitp_ap <- gllvm(sap_tax_otu_final, family = poisson(link = "log"), starting.val = "random")
summary(mydata_fitp_ap)

mydata_fit_ord_nb_ap <- gllvm(sap_tax_otu_final, family = "negative.binomial", starting.val="random")
...
for(i in 1:5){
  fiti_ap_p <- gllvm(sap_tax_otu_final, sap_sample, family = poisson(link = "log"), num.lv=1, sd.errors = FALSE,
...
mydata_fit_env_p_ap <- gllvm(sap_tax_otu_final, sap_sample, family = poisson(link = "log"), num.lv=1,
                           formula = ~ nutrient*sediment*flow*time, seed = 1234)
...
mydata_fit4lv_ap_p <- gllvm(sap_tax_otu_final, family = poisson(link = "log"), num.lv = 1, seed = 1234)
...
mydata_fit_env2_ap_p <- gllvm(sap_tax_otu_final, sap_sample, family = poisson(link = "log"), num.lv = 1, 
                  formula = ~ nutrient*sediment*flow*time, seed = 1234)
...
for(i in 1:5){
  fiti_pp_p <- gllvm(spp_tax_otu_final, spp_sample, family = poisson(link = "log"), num.lv = 4, sd.errors = FALSE,
                     formula = ~ nutrient*sediment*flow*time, seed = 1234)
